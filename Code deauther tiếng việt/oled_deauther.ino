...
void Single() {
  ...
  myfont.print(5, 10, "Đang tấn công Wifi", SSD1306_WHITE, BLACK);
  myfont.print(5, 25, "được chọn .", SSD1306_WHITE, BLACK);
  display.display();
  while (true) {
...
void All() {
  display.clearDisplay();
  myfont.set_font(MakeFont_Font1);
  myfont.print(5, 10, "Đang tấn công tất cả", SSD1306_WHITE, BLACK);
  myfont.print(5, 25, "Wifi quét được .", SSD1306_WHITE, BLACK);
  display.display();
  while (true) {
    if (digitalRead(BTN_OK) == LOW) {
      delay(100);
...
void BecaonDeauth() {
  display.clearDisplay();
  myfont.set_font(MakeFont_Font1);
  myfont.print(5, 10, "Tạo Wifi giả và tấn", SSD1306_WHITE, BLACK);
  myfont.print(5, 25, "công Wifi từ Wifi", SSD1306_WHITE, BLACK);
  myfont.print(5, 40, "quét được .", SSD1306_WHITE, BLACK);
  display.display();
  while (true) {
    if (digitalRead(BTN_OK) == LOW) {
      delay(100);
...
void Becaon() {
  display.clearDisplay();
  myfont.set_font(MakeFont_Font1);
  myfont.print(5, 10, "Tạo Wifi giả từ Wifi", SSD1306_WHITE, BLACK);
  myfont.print(5, 25, "quét được .", SSD1306_WHITE, BLACK);
  display.display();
  while (true) {
    if (digitalRead(BTN_OK) == LOW) {
      delay(100);
...
void attackLoop() {
  ...
  while (running) {
    ...
    const char *attackTypes[] = { "Tấn công", "Tấn công tất cả", "Wifi giả", "Wifi giả + Tấn công", "Quay lại" };
...
void networkSelectionLoop() {
  ...
  while (running) {
    ...
    if (scrollindexx >= menuOffset + 3) menuOffset = scrollindexx - 2;

    for (int i = 0; i < 3 && (menuOffset + i) < totalNetworks; i++) {
      ...
      myfont.set_font(MakeFont_Font1);
      if (isSelected) {
        display.fillRect(2, 13 + i * 17, 104, 16, WHITE);
        display.drawRect(2, 13 + i * 17, 104, 16, BLACK);
        myfont.print(4, 14 + i * 17, displaySSID, BLACK, WHITE);
      } else {
        myfont.print(4, 14 + i * 17, displaySSID, WHITE, BLACK);
      }

      myfont.set_font(MakeFont_Font1);
      myfont.print(108,14 + i * 17,band,SSD1306_WHITE,BLACK);
    }
    display.display();
